...
> 当前版本：**v5.4.22-cmfa.1**（跟随 Clash Party v5.4.22 主线；含借鉴 Proxy-override 批 A+B+D：国内 SDK/CDN 直连 + fake-ip-filter 补全 + direct-nameserver-follow-policy + 节点过滤 junk 关键词补充 + DoH-over-IP bootstrap）
...
本配置共 **22 区域组（11 全部 + 11 家宽）+ 32 业务组**，进入 CMFA「**代理（Proxies）**」页面可见：
